...
export default function ReportDetailPage() {
  ...
  if (error && !report) return (
    <div className="max-w-3xl mx-auto px-4 py-16 text-center">
      ...
      <Link to="/reports" className="text-blue-600 hover:underline text-sm mt-4 inline-block">← Назад кон пријави</Link>
    </div>
  )
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Опис / Description</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Detail label="Поднесена / Submitted" value={report.submittedAt ? new Date(report.submittedAt).toLocaleString('mk-MK') : '—'} icon="🕐" />
            {report.incidentAt && (
              <Detail label="Инцидент / Incident At" value={new Date(report.incidentAt).toLocaleString('mk-MK')} icon="⚠️" />
            )}
            {report.resolvedAt && (
              <Detail label="Решена / Resolved At" value={new Date(report.resolvedAt).toLocaleString('mk-MK')} icon="✅" />
            )}
            <Detail
          ...
          {report.images?.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Слики / Images</h3>
          ...
          {isAdmin() && (
            <div className="border-t pt-5 space-y-3">
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Промени Статус / Change Status</h3>
